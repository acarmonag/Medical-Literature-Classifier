...
class MedTextCleaner(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        # Safer quotes class: includes ' " “ ” ‘ ’ ` ´
        self._rx_quotes = re.compile(r"""['"“”‘’`´]+""")
        # Keep hyphen to preserve tokens like covid-19; drop other symbols
        self._rx_symbols = re.compile(r"[^\w\s\-]+", re.UNICODE)
        self._rx_small_digits = re.compile(r"\b\d{1,3}\b")
        self._rx_bio_hyphen_digits = re.compile(
            r"\b(?:[A-Za-z]+(?:-[A-Za-z]+)*-\d+[A-Za-z]*|\d+[A-Za-z]*(?:-[A-Za-z]+)+)\b"
        )
